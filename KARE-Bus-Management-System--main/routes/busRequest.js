...
router.post('/request', ensureStudentAPI, async (req, res) => {
    try {
        ...
        // Add the request to the user's busRequests array
        await User.findByIdAndUpdate(req.session.user.id, {
            $push: { busRequests: newRequest._id }
        });
    ...
    try {
        ...
        if (io) {
            io.emit('requestStatusUpdate', {
                ...
                busName: request.bus.busName,
                message: 'Your bus request has been accepted!'
            });
        }
    ...
    try {
        ...
        if (io) {
            io.emit('requestStatusUpdate', {
                ...
                status: 'rejected',
                studentId: request.student,
                busName: request.bus ? request.bus.busName : 'Unknown',
                message: 'Your bus request has been rejected.'
            });
        }
