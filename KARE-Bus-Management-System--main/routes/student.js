...
router.get('/dashboard', ensureStudent, async (req, res) => {
  try {
    ...
    // Get user with populated bus assignment and all requests
    ...
    const userData = {
      ...
      pendingBusRequest: user.busRequests && user.busRequests.length > 0 ? 
        user.busRequests.find(req => req.status === 'pending') : null,
      allBusRequests: user.busRequests || []
    };
...
// Bus Details Page
router.get('/bus-details/:id', ensureStudent, async (req, res) => {
  try {
    const busId = req.params.id;
    
    // Find the bus and populate driver information
    const bus = await Bus.findById(busId)
    ...
    // Find all accepted requests for this bus
    ...
    // Find user's request for this bus (if any)
    const userRequest = await BusRequest.findOne({
