...
router.get('/dashboard', ensureDriver, async (req, res) => {
  try {
    ...
    // Get pending bus requests count
    const pendingRequests = bus ? await BusRequest.countDocuments({
      bus: bus._id,
      status: 'pending'
    }) : 0;

    // Get feedback for this driver's bus
    const feedback = bus ? await Feedback.find({ 
      busId: bus._id 
    }).sort({ createdAt: -1 }).limit(10) : [];
    
    // Debug logging
    console.log('Driver ID:', req.session.user.id);
    console.log('Bus found:', bus ? bus._id : 'No bus');
    console.log('Feedback count:', feedback.length);
    if (feedback.length > 0) {
      console.log('First feedback:', feedback[0]);
    }
    ...
    res.render('driver/dashboard', {
      ...
      feedback: []        // Add empty feedback array for error case
    });
  }
});
...
router.get('/passengers', ensureDriver, async (req, res) => {
  try {
    ...
    const formattedPassengers = passengers.map(passenger => ({
      ...
      boardingTime: passenger.boardedTime || passenger.responseTime || passenger.createdAt,
...
router.post('/passengers/board/:id', ensureDriver, async (req, res) => {
  try {
    ...
    // Update the boarded time
    await BusRequest.findByIdAndUpdate(req.params.id, { 
      boardedTime: new Date(),
      status: 'boarded'
    });
    
    res.json({ success: true, message: 'Passenger marked as boarded' });
  } catch (error) {
