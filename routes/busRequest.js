...
router.post('/request', ensureStudentAPI, async (req, res) => {
    try {
        const { busId, boardingStop, destination, forceHtml } = req.body;
        let wantsJson = req.accepts(['json', 'html']) === 'json';
        if (forceHtml) {
            wantsJson = false;
        }
        ...
        if (existingRequest) {
            const message = 'You already have a pending request for this bus';
            if (wantsJson) {
        ...
        const successMessage = 'Bus request submitted successfully';
        if (wantsJson) {
